...
class TemporalDenoiserNetwork(nn.Module):
    ...
    def __init__(self, input_channels=1, base_filters=16):
        ...
        self.temporal = nn.Sequential(
            nn.Conv2d(base_filters * 2, base_filters * 2, (1, 5), 1, (0, 2)), nn.ReLU(),
            nn.Conv2d(base_filters * 2, base_filters * 2, (1, 5), 1, (0, 2)), nn.ReLU(),
        )
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(base_filters * 2, base_filters, 4, 2, 1), nn.ReLU(),
...
class RNNDenoiserNetwork(nn.Module):
    ...
    def __init__(self, input_channels=1, base_filters=16, hidden_size=8, num_layers=1, dropout=0.0):
        ...
        self.conv_encoder = nn.Sequential(
            nn.Conv2d(input_channels, base_filters, 3, 1, 1), nn.ReLU(),
            nn.Conv2d(base_filters, base_filters, 3, 1, 1), nn.ReLU()
        )
        ...
        self.conv_decoder = nn.Sequential(
            nn.Conv2d(hidden_size, base_filters, 3, 1, 1), nn.ReLU(),
    ...
    def forward(self, x):
        ...
        conv_features = conv_features.permute(0, 2, 3, 1)  # (batch, freq, time, base_filters)
        conv_features = conv_features.contiguous().view(batch_size * freq_bins, time_steps, self.base_filters)
...
class UNetDenoiserNetwork(nn.Module):
    ...
    def _match_size(self, x, target):
        ...
        if diff_h > 0 or diff_w > 0:
            ...
            h_start = diff_h // 2
            w_start = diff_w // 2
            x = x[:, :, h_start:h_start + target.shape[2], w_start:w_start + target.shape[3]]
...
class SpeechDenoisingModel:
    ...
    def __init__(self, device="cpu", learning_rate=1e-3, checkpoint_path="checkpoints/best.pth", phase_support=True):
        ...
        self.phase_weight = 0.1  # Weight for phase loss (typically lower than magnitude)
    ...
    def infer(self, noisy_waveform, n_fft, hop_length):
        ...
        if self.phase_support:
            ...
            input_tensor = torch.from_numpy(np.stack([magnitude_log, phase_normalized])).unsqueeze(0).to(self.device)
        else:
        ...
        if self.phase_support:            # Model outputs 2 channels: magnitude and phase
            with torch.no_grad():
                prediction = self.model(input_tensor).cpu().squeeze(0).numpy()  # (2, freq, time)
    ...
    def train_step_with_phase(self, noisy_magnitude, clean_magnitude, noisy_phase, clean_phase):
        """Training step with phase information."""
        self.model.train()
        
        # Stack magnitude and phase as 2-channel input and target
        noisy_input = torch.stack([noisy_magnitude, noisy_phase], dim=1).to(self.device)  # (batch, 2, freq, time)
        clean_target = torch.stack([clean_magnitude, clean_phase], dim=1).to(self.device)  # (batch, 2, freq, time)
    ...
    def evaluate_step_with_phase(self, noisy_magnitude, clean_magnitude, noisy_phase, clean_phase):
        """Evaluation step with phase information."""
        self.model.eval()
        with torch.no_grad():
            # Stack magnitude and phase as 2-channel input and target
            noisy_input = torch.stack([noisy_magnitude, noisy_phase], dim=1).to(self.device)
            clean_target = torch.stack([clean_magnitude, clean_phase], dim=1).to(self.device)
